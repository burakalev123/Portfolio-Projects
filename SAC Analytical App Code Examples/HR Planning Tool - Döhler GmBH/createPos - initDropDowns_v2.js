...
var resultSet = tbl_hidden.getDataSource().getResultSet();
var departments = ArrayUtils.create(Type.string);

for (i = 0; i < resultSet.length; i++) {

    departments.push(resultSet[i]["PD_001_POSITION.T_001_Department"].description);
    drp_Department.addItem(resultSet[i]["PD_001_POSITION.T_001_Department"].id, resultSet[i]["PD_001_POSITION.T_001_Department"].description);

}
console.log("departments");
console.log(departments);

ScriptObject_1.show_Job_Prof_selection();
...
for (i = 0; i < arr_LegalEntity.length; i++) {
    if (arr_LegalEntity[i].displayId !== "Not In Hierarchy" &&
        arr_LegalEntity[i].displayId !== "TOTALSAP" &&
        arr_LegalEntity[i].displayId !== "TOTAL" &&
        arr_LegalEntity[i].displayId !== "TOTALNONSAP" &&
        arr_LegalEntity[i].displayId !== "CIS" &&
        arr_LegalEntity[i].displayId !== "E-APAC" &&
        arr_LegalEntity[i].displayId !== "EUR" &&
        arr_LegalEntity[i].displayId !== "MEA" &&
        arr_LegalEntity[i].displayId !== "NA" &&
        arr_LegalEntity[i].displayId !== "SA" &&
        arr_LegalEntity[i].displayId !== "TIP" &&
        arr_LegalEntity[i].displayId !== "W-APAC" &&
        arr_LegalEntity[i].displayId !== "REGION" &&
        arr_LegalEntity[i].displayId !== "#") {
...
for (i = 0; i < gv_JobProfile.length; i++) {
    if (gv_JobProfile[i].id !== "#") {
        drp_Job_Family.addItem(gv_JobProfile[i].properties["Job_Family"], gv_JobProfile[i].properties["Job_Family"]);
    }
}
// Currency //
