...
SELECT
	*
FROM
	PortfolioProject..CowidDeaths
WHERE
	continent is not null
ORDER BY
	3,
	4 -- Select Data that we are going to be starting with
SELECT
	location,
	date,
	total_cases,
...
FROM
	PortfolioProject..CowidDeaths --WHERE location like 'Netherlands'
GROUP BY
	location,
	population
ORDER BY
	4 DESC -- Countries with Highest Death Count per Population
SELECT
	location,
...
GROUP BY
	location
ORDER BY
	2 DESC -- BREAKING THINGS DOWN BY CONTINENT
...
GROUP BY
	date
ORDER BY
	4 DESC -- Total Population vs Vaccinations
...
SELECT
	...
	SUM(CONVERT(int, vac.new_vaccinations)) OVER (
		...
		ORDER by
			dea.location,
			dea.date
	) as RollingPeopleVaccinated --, (RollingPeopleVaccinated/dea.population)*100 as Percentage
FROM
	PortfolioProject..CowidDeaths dea
	JOIN PortfolioProject..CowidVaccin vac on dea.location = vac.location
	and dea.date = vac.date --WHERE dea.location = 'Turkey'
ORDER BY
	2,
	3 -- USE CTE
	WITH PopvsVac (
